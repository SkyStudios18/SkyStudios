...
public class SheepDataBase extends DatabaseManager {

    public SheepDataBase() {
        super(false);
    }
        ...
        try {
            ...
            while (rs.next()) {
                if (rs.getString("player").equals(player.getUniqueId().toString())) {
                    return Integer.parseInt(rs.getString("sheepcounter"));
                }
            }
    ...
    public void addPlayer(Player player, int increment) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                ps1 = getConnection().prepareStatement("UPDATE HeadData SET sheepcounter = ? WHERE player = ?");
                ...
                ps1.setString(4, Integer.toString(increment));
